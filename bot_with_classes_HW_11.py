...
COMMAND_WORDS = ["add", "change", "phone", "show all", "help", "birthday", "days_to_birthday"]
...
class Record(Field):
    ...
    def add_user(self, name, phones, address_book, birthday = None):
        ...
        new_user[name] = {"name": name, "phones": phones, "birthday": birthday}
        ...
        for key, val in address_book.items(): 
            if key == name and address_book[key]["birthday"]:
                ...
                if self.next_birthday - self.date_today < timedelta(days=0):
                    ...
                    difference = str(self.next_birthday-self.date_today).strip(",")
                    return f"The {key}'s birthday will be in {difference[0]}"
            ...
            if user_command_small_letters.startswith(word):
                ...
                if user_data[0].lower() == "add":
                    ...
                    print("""Enter\n
                          ...
                          'show all' and quantity of users on page to see all users""")

                elif user_command_small_letters.startswith("show all"):
                    if len(user_data) > 2:
                        pages = contacts.get_contacts_pages(int(user_data[2]))
                        if len(contacts.get_contacts())%int(user_data[2]):
                            for i in range(len(contacts.get_contacts())//int(user_data[2]) + 1):
                                print(f"page {i} {pages[i]}")
                        else:
                            for i in range(len(contacts.get_contacts())//int(user_data[2])):
                                print(f"page {i} {pages[i]}")
